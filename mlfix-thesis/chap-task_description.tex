...
they contained too many errors, or the errors were of a different nature than the ones we detect).

We have evaluated set of \todo{how much} sentences extracted from the WMT16\footnote{Specifically, we have used a dataset
where the MT output was created by the Chimera system.} dataset. We have also extracted
...
In table\todo{ref} we present the results of the manual evaluation. Only the sentences, that were
modified by the Oracle classifier were annotated by the human evaluator.
%todo: prec recall ala depfix
